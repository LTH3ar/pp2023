...
class Input:
    ...
    def load_data_student(self):
        with open('student_data.dt', 'rb') as file:
    ...
    def load_data_course(self):
        with open('course_data.dt', 'rb') as file:
            course_data = pickle.load(file)
            for course in course_data:
                course_id = str(course.get_id())
                course_name = str(course.get_name())
                course_credit = str(course.get_credit())
                new_course = Course()
                new_course.set_id(course_id)
                new_course.set_name(course_name)
                new_course.set_credit(course_credit)

                for student_id, mark in course.get_full_mark().items():
                    new_course.input_mark(str(student_id), str(mark))
